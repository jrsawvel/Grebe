...
### Note
$hash{post_text} = "$date_time this will be the title for a note that is intentionally longer than max note title len and max note title len which is more token than an acutal title since notes are viewed as titleless which suggests that maybe this feature should not exist in this blogging app.\n\nhere is the start of the body text\n\nhere is some more text.\n\n testin **markdown bolding**\n\nhashtag #test #perl";
...
ok($rc >= 200 && $rc < 300, 'note successfully created.');

open($fh, ">", "t/note-info.json") or die "cannot open > note-info.json: $!";
